      ...
      <div className="list-card glass-panel" style={{ maxWidth: '900px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <button 
              style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-color)', background: filter === 'all' ? 'rgba(58, 13, 22, 0.1)' : 'transparent', color: filter === 'all' ? 'var(--primary-color)' : 'var(--text-secondary)', fontWeight: filter === 'all' ? '600' : '400', cursor: 'pointer' }}
              onClick={() => setFilter('all')}
            >
              All
            </button>
            <button 
              style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-color)', background: filter === 'income' ? 'rgba(58, 13, 22, 0.1)' : 'transparent', color: filter === 'income' ? 'var(--primary-color)' : 'var(--text-secondary)', fontWeight: filter === 'income' ? '600' : '400', cursor: 'pointer' }}
              onClick={() => setFilter('income')}
            >
              Income
            </button>
            <button 
              style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-color)', background: filter === 'expense' ? 'rgba(58, 13, 22, 0.1)' : 'transparent', color: filter === 'expense' ? 'var(--primary-color)' : 'var(--text-secondary)', fontWeight: filter === 'expense' ? '600' : '400', cursor: 'pointer' }}
              onClick={() => setFilter('expense')}
            >
              Expenses
            </button>
            <button 
              style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-color)', background: filter === 'alert' ? 'rgba(58, 13, 22, 0.1)' : 'transparent', color: filter === 'alert' ? 'var(--primary-color)' : 'var(--text-secondary)', fontWeight: filter === 'alert' ? '600' : '400', cursor: 'pointer' }}
